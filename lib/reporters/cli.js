...
Reporter.prototype.benchEnd = function (name, results) {
  ...
  var memory = ((results.memory.end.heapUsed - results.memory.start.heapUsed) / 1048576).toFixed(3) + ' MB'
    , ops = (results.iterations / results.elapsed * 1000).toFixed(0)
    , elpsd = results.elapsed;
  
  console.log('  %s '.grey + humanize(results.iterations), pad('iterations', w));
  console.log('  %s '.grey + humanize(ops), pad('ops/s:', w));
  //console.log('  %s '.grey + memory, pad('mem:', w));
  console.log('  %s '.grey + elpsd + 'ms', pad('elapse:', w));
  console.log('');
};
